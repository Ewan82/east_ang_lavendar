...
class FourDEnVar:
    ...
    :param run_xb: switch to run JULES for the ensemble values specified in experiment_setup module (bool)
    ...
    def __init__(self, assim=False, seed_val=es.seed_value):
        # set parameters to optimize and ensemble values
        self.p_dict = es.opt_params
        self.p_keys = list(self.p_dict.keys())
        self.p_keys.sort()
        self.xb = []
        self. val_bnds = []
        for param in self.p_keys:
            self.xb.append(self.p_dict[param][0])
            self.val_bnds.append(self.p_dict[param][1])
        ...
        # set ensemble error and generate ensemble ensemble
    ...
    def make_obs(self):
        ...
        self.yoblist = obs_dic['sm_obs_t']
        self.rmat_lst = obs_dic['sm_rmats']

    def make_hxb(self):
        # creates hxb and Xb ensemble matrix given function in experiment_setup module
        #hxb_dic = es.jules_hxb()
        self.hxb_lst = es.jules_hxb()  # hxb_dic['obs']
    ...
    def create_ensemble(self):
        ...
        self.hm_xbs_lst = es.jules_hxb_ens()
        self.hmx_mat_lst = [(1. / (np.sqrt(self.size_ens - 1))) *
                        np.array([hmxb[1][x] - self.hxb_lst[x] for hmxb in self.hm_xbs_lst])
                        for x in range(len(self.hxb_lst))]
    ...
    def obcost_ens_inc(self, wvals):
        ...
        """
        return np.sum([np.dot(np.dot((np.dot(self.hmx_mat_lst[x].T, wvals) + self.hxb_lst[x] - self.yoblist[x]),
                              np.linalg.inv(self.rmat_lst[x])),
                       (np.dot(self.hmx_mat_lst[x].T, wvals) + self.hxb_lst[x] - self.yoblist[x]).T)
                for x in range(len(self.hxb_lst))])
    ...
    def cost_ens_inc(self, wvals):
        ...
        :param wvals: array of ensemble weights, w
        :return: value of the ensemble part of the cost function as float
        """
    ...
    def gradcost_ens_inc(self, wvals):
        ...
        #                                   (np.dot(self.hmx_mat.T, wvals) + self.hxb - self.yoblist).T))
        obcost = np.sum([np.dot(self.hmx_mat_lst[x],
                                np.dot(np.linalg.inv(self.rmat_lst[x]),
                                       (np.dot(self.hmx_mat_lst[x].T, wvals) + self.hxb_lst[x] - self.yoblist[x]).T))
                         for x in range(len(self.hxb_lst))])
    ...
    def cost(self, xi, xi_nc):
        ...
        obcost = np.sum([np.dot(np.dot((hxi[x] - self.yoblist[x]), np.linalg.inv(self.rmat_lst[x])),
                      (hxi[x] - self.yoblist[x]).T)for x in range(len(self.hxb_lst))])
    ...
    def a_cov(self):
        ...
        """
        a_cov = np.linalg.inv(splinal.sqrtm(np.eye(self.size_ens) + np.sum([np.dot(self.hmx_mat_lst[x],
                        np.dot(np.linalg.inv(self.rmat_lst[x]), self.hmx_mat_lst[x].T))
                                                                            for x in range(len(self.hxb_lst))])))
        return a_cov
    ...
    def a_ens(self, xa):
        ...
        xa_mat = np.dot(self.xb_mat.T, nplin.cholesky(a_cov)).T
    ...
    def perturb_true_state(self, x_truth, err=0.05):
        ...
        experiments to find a ensemble parameter vecotr (self.xb)
